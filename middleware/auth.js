...
module.exports = function(req, res, next) {// middleware function; function that has access to request and response objects; next is callback that have to run once done so that it moves on to next piece of middleware
    ...
    //verify token
    try {// if valid will decode token
        const decoded = jwt.verify(token, process.env.jwtSecret);//decode token

        req.user = decoded.user;
        next();
    } catch(err) { // if there is token but it is not valid
