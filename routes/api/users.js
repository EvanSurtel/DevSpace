...
router.post(
  ...
  async (req, res) => {
    //this handles the response to the validation
    ...
    try {
      ...
      user = new User({
        //create new instance of user;*doesnt save user; we have to call user.save to save
        name,
        email,
        avatar,
        password,
      ...
      const payload = {
        // need to create payload; then sign jwt and pass in payload; then callback to send response to client with the token; then protect route with middleware that will verify token
        user: {
